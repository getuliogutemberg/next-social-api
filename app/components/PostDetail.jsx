      ...
      {posts.filter((post) => post.id === props.id).map((post)=>{
        ...
        return (
          ...
          <div ref={commentsContainerRef} className="bg-gray-100 rounded-lg  shadow-md p-4 flex flex-col justify-between  w-[100%]  mb-4 overflow-auto overflow-x-hidden " >
            ...
            <p onClick={() => JSON.parse(localStorage.getItem('user_id')) !== post.createdBy.id && handleLike(post)} className=" cursor-pointer text-red-400 text-bold hover:scale-150 flex gap-4">{post.likes.filter((like)=>like.id === JSON.parse(localStorage.getItem('user_id'))).length > 0 ? <AiFillHeart className='scale-150'/> : post.likes.length !== 0 ? <AiFillHeart  className='scale-150'/> : <AiOutlineHeart  className='scale-150'/>}<p className='text-slate-900 text-xl'>{post.likes.length}</p></p>
